'''
parallel-nbasim.py:
    fantasy basketball season simulator
...
class NBASim(object):
    ...
    def sim(self, frame, iterations=500, num_players = 10, num_teams = 10):
        ...
        frame['pid'] = range(0, len(frame))
        frame['TO'] = 0 - frame['TO']
        frame['team_id'] = None

        # calculate reused values outside of loop
        num_cores = multiprocessing.cpu_count()
        n = num_teams * num_players
        prange = range(0, num_players)
        tid_range = prange * num_teams
        value_columns = ['FGM', 'FGA', 'FTM', 'FTA', '3M', 'TR', 'AS', 'ST', 'BK', 'PTS', 'TO']
        rank_columns = ['FGP', 'FTP', '3M', 'TR', 'AS', 'ST', 'BK', 'PTS', 'TO']

        # parallelize
        results = Parallel(n_jobs=num_cores)(delayed(nbasim)(frame, i, n, prange, tid_range, value_columns, rank_columns) for i in xrange(1, iterations+1))
